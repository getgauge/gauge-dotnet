...
public class ExecuteStepProcessor : IGaugeProcessor<ExecuteStepRequest, ExecutionStatusResponse>
{
    ...
    public async Task<ExecutionStatusResponse> Process(int streamId, ExecuteStepRequest request)
    {
        ...
        var protoExecutionResult = await _executionOrchestrator.ExecuteStep(method, streamId, args);
